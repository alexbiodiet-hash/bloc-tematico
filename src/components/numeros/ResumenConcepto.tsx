...
export default function ResumenConcepto({ registros, unidad }: Props) {
  ...
  const total = useMemo(() => filtrados.reduce((s, r) => s + r.valor, 0), [filtrados])
  const media = filtrados.length ? total / filtrados.length : 0
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 p-3 text-center">
          <div className="text-xs text-emerald-500 dark:text-emerald-400 mb-1 font-medium">MEDIA</div>
          <div className="text-lg font-bold text-emerald-700 dark:text-emerald-300">
            {count ? fmt(media) : '—'}{count ? u : ''}
          </div>
        </div>
        <div className="rounded-lg bg-amber-50 dark:bg-amber-900/20 p-3 text-center">
          <div className="text-xs text-amber-500 dark:text-amber-400 mb-1 font-medium">REGISTROS</div>
          <div className="text-lg font-bold text-amber-700 dark:text-amber-300">{count}</div>
        </div>
      </div>
    </div>
  )
